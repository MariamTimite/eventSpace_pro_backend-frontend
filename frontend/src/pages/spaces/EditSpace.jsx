...
const EditSpace = () => {
  ...
  useEffect(() => {
    ...
    spaceService.getById(id)
      .then(res => {
        const space = res.data.data || res.data.space;
        setFormData({
          name: space.name || '',
          ...
          price: space.price || '',
          priceType: space.priceType || 'HEURE',
          amenities: (space.amenities || []).join(', '),
          address: typeof space.address === 'string' ? space.address : JSON.stringify(space.address || {}),
          coordinates: typeof space.coordinates === 'string' ? space.coordinates : JSON.stringify(space.coordinates || {}),
          images: [],
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess('Espace modifié avec succès !');
      setTimeout(() => navigate('/spaces'), 1500);
    } catch (err) {
      setError(err.response?.data?.message || "Erreur lors de la modification de l'espace");
    } finally {
      setLoading(false);
    }
  };
    ...
    <ProtectedRoute roles={['ADMIN']}>
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 py-8 px-4">
        <div className="bg-white shadow-xl rounded-lg max-w-2xl w-full p-8 space-y-8">
          <h2 className="text-3xl font-extrabold text-center text-gray-900 mb-2">Modifier l'espace</h2>
          <p className="text-center text-gray-500 mb-6">Modifiez les champs puis validez pour mettre à jour l'espace.</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input type="file" name="images" accept="image/*" multiple onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" />
                <span className="text-xs text-gray-400">Vous pouvez ajouter de nouvelles images</span>
